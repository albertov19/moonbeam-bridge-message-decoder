...
fn format_instruction(instr: &Instruction<()>) -> String {
    match instr {
        ...
        Instruction::SetAppendix(xcm) => {
            let inner_instrs: Vec<String> = xcm.0.iter().map(format_instruction).collect();
            format!("SetAppendix(Xcm([{}]))", inner_instrs.join(", "))
        }
        Instruction::SetErrorHandler(xcm) => {
            let inner_instrs: Vec<String> = xcm.0.iter().map(format_instruction).collect();
            format!("SetErrorHandler(Xcm([{}]))", inner_instrs.join(", "))
        }
        Instruction::RefundSurplus => "RefundSurplus".to_string(),
